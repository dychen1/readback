...
public actor ChatterboxTurboRuntimeAdapter: SpeechRuntimeAdapter {
    public let kind = SpeechRuntimeKind.chatterboxTurbo
    ...
    public func validateModel(at directory: URL) async throws {
        ...
        guard config.modelType?.lowercased() == "chatterbox_turbo" else {
            throw SpeechRuntimeAdapterError.unsupportedModelConfiguration(.chatterboxTurbo)
        }
    }
    ...
    public func synthesize(
        ...
        guard let model else {
            throw SpeechRuntimeAdapterError.modelNotLoaded(.chatterboxTurbo)
        }
