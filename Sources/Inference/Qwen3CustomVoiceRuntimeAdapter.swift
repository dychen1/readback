...
public actor Qwen3CustomVoiceRuntimeAdapter: SpeechRuntimeAdapter {
    public let kind = SpeechRuntimeKind.qwen3CustomVoice
    ...
    public func validateModel(at directory: URL) async throws {
        ...
        guard config.modelType?.lowercased() == "qwen3_tts",
              config.ttsModelType?.lowercased() == "custom_voice"
        else {
            throw SpeechRuntimeAdapterError.unsupportedModelConfiguration(.qwen3CustomVoice)
        }
    }
    ...
    public func synthesize(
        ...
        guard let model else {
            throw SpeechRuntimeAdapterError.modelNotLoaded(.qwen3CustomVoice)
        }
