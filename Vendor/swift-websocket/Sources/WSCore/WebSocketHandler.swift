...
@_spi(WSInternal) public actor WebSocketHandler {
                    ...
                    if case .closing = self.stateMachine.state {
                        group.addTask {
                            ...
                            try await clock.sleep(
                                until: clock.now.advanced(by: self.configuration.closeTimeout)
                            )
    ...
    func runAutoPingLoop() async throws {
        ...
        let clock = ContinuousClock()
        try await clock.sleep(until: clock.now.advanced(by: period))
        while true {
            ...
            case .wait(let time):
                try await clock.sleep(until: clock.now.advanced(by: time))
